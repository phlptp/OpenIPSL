within OpenIPSL.ElectricalFMU.Controls.PSAT.OEL;
model OEL "PSAT Over Excitation Limiter"
  Modelica.Blocks.Interfaces.RealInput v "Generator terminal voltage (pu)"
    annotation (Placement(transformation(extent={{-110,68},{-90,88}}),
        iconTransformation(extent={{-90,66},{-62,98}})));
  Modelica.Blocks.Interfaces.RealInput p "Active power (pu)" annotation (
      Placement(transformation(extent={{-110,36},{-90,56}}), iconTransformation(
          extent={{-90,32},{-62,62}})));
  Modelica.Blocks.Interfaces.RealInput q "Reactive power (pu)" annotation (
      Placement(transformation(extent={{-110,4},{-90,24}}), iconTransformation(
          extent={{-90,-6},{-62,24}})));
  FieldCurrent field_current(xd=xd, xq=xq)
    annotation (Placement(transformation(extent={{-82,38},{-46,72}})));
  Modelica.Blocks.Sources.Constant set(k=iflim)
    annotation (Placement(transformation(extent={{-40,0},{-20,20}})));
  //Ymin=0,
  //nStartValue=0,
  //Ymax=vOELmax,
  //K=1/T0)
  Modelica.Blocks.Interfaces.RealOutput v_OXL annotation (Placement(
        transformation(extent={{98,28},{118,48}}), iconTransformation(extent={{
            66,6},{94,38}})));
  ...
  Modelica.Blocks.Sources.Constant const(k=iflim)
  ...
  Modelica.Blocks.Sources.Constant set1(k=0)
...
equation
  ...
  connect(q, field_current.q) annotation (Line(
      ...
      color={0,0,127},
      smooth=Smooth.None));
  connect(field_current.ifield, add.u1) annotation (Line(points={{-50.68,56.19},
          {-48,56.19},{-48,56},{-42,56}}, color={0,0,127}));
  connect(const.y, add.u2) annotation (Line(points={{-53,20},{-48,20},{-48,44},
          {-42,44}}, color={0,0,127}));
  connect(add.y, greater.u1)
    annotation (Line(points={{-19,50},{-10.5,50},{-2,50}}, color={0,0,127}));
  connect(set.y, greater.u2) annotation (Line(points={{-19,10},{-10,10},{-10,42},
          {-2,42}}, color={0,0,127}));
  connect(greater.y, switch1.u2) annotation (Line(points={{21,50},{26,50},{26,
          10},{36,10}}, color={255,0,255}));
  connect(switch1.u1, greater.u1) annotation (Line(points={{36,18},{-6,18},{-6,
          50},{-2,50}}, color={0,0,127}));
  connect(set1.y, switch1.u3) annotation (Line(points={{-19,-30},{20,-30},{20,2},
          {36,2}}, color={0,0,127}));
  connect(switch1.y, limIntegrator.u)
    annotation (Line(points={{59,10},{66,10}}, color={0,0,127}));
  ...
  annotation (
    ...
    Icon(coordinateSystem(preserveAspectRatio=false, extent={{-100,-100},{100,
            100}}), graphics={Rectangle(extent={{-60,92},{68,-48}}, lineColor={
          0,0,255}),Text(
          extent={{-68,100},{-30,64}},
          lineColor={0,0,255},
